...
def list_threagile_issues(repo, token):
    """Return a dict of {label_name: issue} for all issues labelled with threagile:."""
    path = f"/repos/{repo}/issues?labels=threagile&state=all&per_page=100"
    # GitHub label filter requires the label to exist; use the prefix approach:
    # fetch all issues with any threagile: label by filtering client-side.
...
def sync(args):
    ...
    risks = [r for r in risks
             if severity_order.index(r.get("severity", "low").lower()
                                     if r.get("severity", "low").lower() in severity_order
                                     else "low") >= min_idx]
    ...
    # Fetch existing threagile-tracked issues
    ...
    # Index current risks by synthetic ID
    ...
    # Close issues for risks that no longer appear in the current run
