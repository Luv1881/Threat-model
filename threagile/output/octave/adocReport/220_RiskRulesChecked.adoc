...
| Threagile Execution Timestamp: | 20260604113051
...
== OCTAVE Allegro - Information Asset Container Without Audit Logging
[.small]#octave-insufficient-logging#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Repudiation
| Description: | OCTAVE Allegro requires organisations to identify and evaluate threats to their information assets. Without audit logging on information asset containers, there is no forensic record of who accessed, modified, or deleted organisational data. This makes it impossible to detect insider threats, investigate breaches, or demonstrate compliance with data governance requirements.

| Detection:   | In-scope technical assets tagged with information-asset-container that are not tagged with has-audit-logging.

| Rating:      | Medium baseline for all information asset containers. Elevated to high when the asset holds confidential or strictly-confidential data.
...
== OCTAVE Allegro - High-Criticality Information Asset Container Without Automated Backup
[.small]#octave-no-backup#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | OCTAVE Allegro identifies availability as a core security concern for high-criticality information assets. A technical asset tagged as both an information-asset-container and criticality-high that does not have automated backup lacks the resilience needed to recover from accidental deletion, ransomware, corruption, or hardware failure. Loss of data with no backup results in permanent destruction of organisational information assets.

| Detection:   | In-scope technical assets tagged with information-asset-container and criticality-high that are not tagged with has-automated-backup.

| Rating:      | High baseline for criticality-high information asset containers without backup. Elevated when the asset has no alternative data source and recovery is impossible.
...
== OCTAVE Allegro - Information Asset Container Transmits Data Without Encryption
[.small]#octave-unencrypted-transport#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | OCTAVE Allegro requires evaluation of how information assets are transported between systems. A technical asset tagged as an information-asset-container whose encryption field is set to none may transmit or expose data over unencrypted channels. Network observers, rogue infrastructure, or man-in-the-middle positions can intercept and read organisational information assets in transit.

| Detection:   | In-scope technical assets tagged with information-asset-container whose encryption field is set to none.

| Rating:      | High baseline for all information asset containers with unencrypted transport. Critical when the asset stores strictly-confidential or personally identifiable data.

|===

== OCTAVE Allegro - Internet-Accessible Information Asset Container Without Encryption at Rest
[.small]#octave-cross-zone-storage#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | OCTAVE Allegro requires assessment of information assets that cross organisational or network zone boundaries. A technical asset tagged as information-asset-container that is directly accessible from the internet without encryption at rest combines two high-risk attributes: internet exposure and plaintext storage. If the asset is compromised via a misconfiguration, credential theft, or vulnerability, all stored data is immediately readable without any additional decryption step.

| Detection:   | In-scope technical assets tagged with information-asset-container that are internet-accessible and not tagged with has-encryption-at-rest.

| Rating:      | Critical for all internet-accessible information asset containers without encryption at rest. No severity reduction applies when both attributes are present simultaneously.
...
== OCTAVE Allegro - High-Criticality Multi-Tenant Information Asset Container Without Privileged Access Management
[.small]#octave-insider-threat#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | OCTAVE Allegro specifically addresses insider threats as a primary area of concern for information asset security. A multi-tenant technical asset tagged as information-asset-container and criticality-high gives multiple tenants access to the same underlying infrastructure. Without Privileged Access Management (PAM) — evidenced by the absence of the has-privileged-access-management tag — privileged operators and service accounts can access all tenant data without individual accountability or just-in-time approval workflows.

| Detection:   | In-scope technical assets tagged with information-asset-container and criticality-high that have multi_tenant set to true and are not tagged with has-privileged-access-management.

| Rating:      | High baseline for all qualifying assets. Elevated to critical when the asset holds personally identifiable, financial, or regulated data across tenants.

|===

== OCTAVE Allegro - High-Criticality Information Asset Container Without Disaster Recovery Plan
[.small]#octave-no-recovery-plan#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | OCTAVE Allegro evaluates organisational resilience alongside confidentiality threats. A high-criticality information asset container without a tested disaster recovery plan leaves the organisation unable to restore operations within an acceptable recovery time objective (RTO) following a destructive event such as ransomware, datacenter failure, or catastrophic data corruption. The OCTAVE Allegro area-of-concern for availability requires explicit recovery planning for all critical information assets.

| Detection:   | In-scope technical assets tagged with information-asset-container and criticality-high that are not tagged with has-disaster-recovery.

| Rating:      | High baseline for all high-criticality information asset containers without DR. Elevated when the asset is the sole source of truth for critical business data.

|===

== OCTAVE Allegro - Information Asset Shared With Third Party Without Data Processing Agreement
[.small]#octave-third-party-exposure#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | OCTAVE Allegro requires organisations to assess areas of concern around how information assets are accessed and shared. A technical asset tagged as both information-asset-container and third-party-shared indicates that organisational data is shared with an external party. Without a Data Processing Agreement (DPA) in place — evidenced by the has-dpa tag — there are no contractual guarantees over how the third party processes, stores, or protects the data.

| Detection:   | In-scope technical assets tagged with information-asset-container and third-party-shared that are not tagged with has-dpa.

| Rating:      | High baseline. Elevated to critical when the asset contains personal data subject to GDPR or sector-specific data protection regulations.
...
== OCTAVE Allegro - Information Asset Container Without Access Control
[.small]#octave-no-access-control#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | OCTAVE Allegro focuses on information assets and the containers that hold them. A technical asset tagged as an information-asset-container that has no access control mechanisms allows any principal with network reachability to read or modify the stored information. This violates the core OCTAVE Allegro protection requirement of restricting access to information assets to only authorised parties.

| Detection:   | In-scope technical assets tagged with information-asset-container that are not tagged with has-access-control.

| Rating:      | High baseline for all information asset containers. Elevated to critical when the asset holds strictly-confidential or mission-critical data.
