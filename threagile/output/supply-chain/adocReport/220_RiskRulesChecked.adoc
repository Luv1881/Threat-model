...
| Threagile Execution Timestamp: | 20260604113047
...
== Supply Chain - Container Platform Running Unpinned Base Images
[.small]#supply-chain-unpinned-base-images#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Container platforms that run images referenced by mutable tags (e.g. :latest, :stable, :production) without digest pinning are vulnerable to silent base image substitution. If a base image tag is updated by the upstream publisher to include a vulnerability or a supply-chain backdoor, the next pull silently uses the compromised version. Digest pinning (sha256: prefix) ensures the exact image content is always used, regardless of tag changes.

| Detection:   | In-scope technical assets with container-platform technology not tagged with has-pinned-base-images.

| Rating:      | Medium baseline. Elevated when the container platform pulls from external public registries that are not under the organization's control.
...
== Supply Chain - Source Code Merged Without Mandatory Peer Review
[.small]#supply-chain-no-code-review#
...
| Detection:   | In-scope technical assets with sourcecode-repository technology not tagged with has-required-review.

| Rating:      | High for repositories containing production application code. Medium for infrastructure-as-code repositories.

|===

== Supply Chain - Build Pipeline Without SLSA Provenance Attestation
[.small]#supply-chain-no-build-provenance#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Build pipelines that do not generate SLSA (Supply-chain Levels for Software Artifacts) provenance attestations allow build artifacts to be tampered with between build and deployment with no way to detect or prove the tampering. SLSA L2+ provenance provides a cryptographically signed record of who built what, when, from which source commit, using which build tool.

| Detection:   | In-scope technical assets with build-pipeline technology that produce custom-developed artifacts and are not tagged with has-slsa-provenance.

| Rating:      | High for externally distributed software and CI/CD pipelines deploying to production. Medium for internal tooling.
...
== Supply Chain - Custom-Developed Application Without Static Analysis (SAST)
[.small]#supply-chain-no-sast#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Applications with custom-developed code that do not run Static Application Security Testing (SAST) during CI/CD miss a class of security vulnerabilities that are detectable by automated analysis: SQL injection, XSS, path traversal, hardcoded secrets, use of deprecated cryptographic functions, and many OWASP Top 10 patterns. SAST tools (Semgrep, CodeQL, SonarQube, Bandit, ESLint security plugins) provide continuous vulnerability detection at the lowest cost point — before code ships.

| Detection:   | In-scope technical assets with custom_developed_parts set to true that do not have the has-sast tag.

| Rating:      | High for externally-accessible applications processing authentication or sensitive data. Medium for internal tools.

|===

== Supply Chain - Build Pipeline Without Software Bill of Materials
[.small]#supply-chain-no-sbom#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Build pipelines that do not generate a Software Bill of Materials (SBOM) in CycloneDX or SPDX format make it impossible to audit third-party dependencies after a software supply chain incident (e.g. Log4Shell, XZ Utils). Without a machine-readable inventory of all components and their versions, vulnerability scanning tools cannot identify which builds are affected by a newly disclosed CVE.

| Detection:   | In-scope technical assets with build-pipeline technology that are not tagged with has-sbom.

| Rating:      | Medium baseline. Elevated when the build pipeline produces software distributed externally (EU CRA, FDA Pre-Market Cybersecurity) or used in critical infrastructure.

|===

== Supply Chain - Build Pipeline Without Dependency Vulnerability Scanning
[.small]#supply-chain-no-dependency-scanning#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Build pipelines that do not run automated dependency scanning (SCA) during CI/CD will ship known-vulnerable third-party libraries into production. Dependency scanning tools (Dependabot, Snyk, OWASP Dependency-Check, Grype) compare the dependency manifest against CVE databases and block merges that introduce or retain high-severity vulnerabilities.

| Detection:   | In-scope technical assets with build-pipeline technology not tagged with has-dependency-scanning.

| Rating:      | High when the build pipeline produces externally distributed or internet-facing software. Medium for internal tools.
...
== Supply Chain - Source Code Repository Without Branch Protection
[.small]#supply-chain-no-branch-protection#
...
| Detection:   | In-scope technical assets with sourcecode-repository technology not tagged with has-branch-protection.

| Rating:      | High for repositories containing production code or secrets. Medium for documentation or configuration repositories.
...
== Supply Chain - Artifact Registry Publicly Accessible Without Authentication
[.small]#supply-chain-artifact-registry-unauthenticated#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Artifact registries (npm, Maven Central mirrors, PyPI proxies, Nexus, Artifactory) that allow unauthenticated write access or that proxy public package repositories without validation are vulnerable to dependency confusion attacks. An attacker can publish a malicious package with a name matching an internal private package, and the build tool will pull the attacker's version from the public registry if it has a higher version number than the internal one.

| Detection:   | In-scope technical assets with artifact-registry technology that are internet-accessible and not tagged with has-private-registry.

| Rating:      | High when the registry is used in production build pipelines. Medium for development-only registries.
...
== Supply Chain - Container Images Without Cryptographic Signing
[.small]#supply-chain-unsigned-container-image#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Container images that are not cryptographically signed (via Cosign/Sigstore, Notary v2, or Docker Content Trust) can be replaced by malicious versions in the container registry without detection. An attacker with registry write access can swap an image tag to point to a backdoored image, and without signature verification in the deployment pipeline, the malicious image will be deployed silently.

| Detection:   | In-scope technical assets with container-platform or container-registry technology that do not have the has-signed-images tag.

| Rating:      | High for container platforms running internet-facing or production workloads. Medium for development/staging platforms with restricted access.
...
== Supply Chain - Build Pipeline With Potentially Exposed Secrets
[.small]#supply-chain-build-secrets-exposure#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Build pipelines that store secrets (API keys, deploy credentials, signing keys) as unmasked environment variables or in plaintext configuration files expose those secrets to every build job, log file, and engineer with pipeline read access. Exposed build secrets are a primary pivot path after CI/CD compromise — an attacker who gains access to build logs or pipeline environment can extract credentials for all downstream services the pipeline can reach.

| Detection:   | In-scope technical assets with build-pipeline technology that have the may_contain_secrets attribute set and are not tagged with has-secrets-manager.

| Rating:      | High when the build pipeline has access to production credentials. Medium for development-only pipelines with limited blast radius.
