...
| Threagile Execution Timestamp: | 20260604113053
...
== Trike - High-Trust Actor Without Audit Logging
[.small]#trike-high-trust-unmonitored#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Trike assigns high trust levels to actors with broad permissions over system assets. A technical asset tagged with trike-trust-level-admin that lacks the has-audit-logging tag has no audit trail for the operations it performs. High-trust actors are the highest-risk principals in any system: their actions are by definition authorised, making detection of abuse entirely dependent on after-the-fact log analysis. Without logging, insider misuse and compromised admin accounts cannot be detected or investigated.

| Detection:   | In-scope technical assets tagged with trike-trust-level-admin that are not tagged with has-audit-logging.

| Rating:      | High baseline for all unmonitored admin-trust actors. Critical when the actor has access to datastores containing strictly-confidential or regulated data.

|===

== Trike - Untrusted Actor With Outgoing Links to Datastores Without Authorised Write
[.small]#trike-unauthorized-write#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Trike's action matrix explicitly models which actors are permitted to perform write (create, update, delete) operations on each asset. A technical asset tagged with trike-actor-untrusted that has outgoing communication links targeting datastores without the trike-write-authorized tag has not had its write rights formally approved. Unauthorised write access allows untrusted actors to corrupt, overwrite, or destroy data in high-value datastores.

| Detection:   | In-scope technical assets tagged with trike-actor-untrusted that have at least one outgoing communication link to a datastore and are not tagged with trike-write-authorized.

| Rating:      | High baseline. Critical when the target datastore holds strictly-confidential, financial, or personally identifiable data where integrity is essential.

|===

== Trike - High-Confidentiality Datastore Without Action Matrix Review
[.small]#trike-asset-no-action-matrix#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Trike's rights-based methodology centres on the action matrix, which defines which actors are permitted to perform create, read, update, and delete (CRUD) operations on each asset. A datastore with high confidentiality that lacks the trike-action-matrix-reviewed tag has never had its permitted actions formally reviewed. Without this review, actors may possess unintended read or write rights to confidential data, and the system's intended access rights have not been verified against the implemented controls.

| Detection:   | In-scope technical assets of type datastore with high or stricter confidentiality that are not tagged with trike-action-matrix-reviewed.

| Rating:      | High for datastores with strictly-confidential data. Medium for confidential data. Elevated when the datastore is accessed by untrusted or externally-facing actors.
...
== Trike - Information Asset Container With High Sensitivity Without Formal Residual Risk Acceptance
[.small]#trike-residual-risk-unaccepted#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Trike's risk treatment process requires that after all feasible mitigations have been applied, any remaining residual risk must be formally accepted by the asset owner and documented in the risk register. A technical asset tagged with information-asset-container that also has high or stricter confidentiality but lacks the trike-residual-risk-accepted tag has not completed this final step. An unaccepted residual risk indicates the organisation has not formally acknowledged the remaining exposure, which may leave known threats unaddressed and management without visibility of accepted risk.

| Detection:   | In-scope technical assets tagged with information-asset-container that have high or stricter confidentiality and are not tagged with trike-residual-risk-accepted.

| Rating:      | Medium baseline indicating incomplete risk governance. Elevated when the asset holds strictly-confidential or regulated data where governance failures carry direct regulatory consequences.
...
== Trike - Multi-Tenant Actor With Accumulated Privileges Across Datastores Without Privilege Review
[.small]#trike-privilege-accumulation#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Trike's rights-based model requires that actor privileges be explicitly scoped and reviewed. A multi-tenant technical asset tagged with trike-actor that lacks the trike-privilege-review tag has never had its cumulative access rights audited. Multi-tenant actors that accumulate access to multiple high-value datastores without a privilege review may hold more permissions than necessary, violating the principle of least privilege and creating a high-impact pivot point if the actor is compromised.

| Detection:   | In-scope multi-tenant technical assets tagged trike-actor that are not tagged with trike-privilege-review.

| Rating:      | High when the actor has access to datastores containing confidential or critical data. Elevated to critical when the actor is also internet-facing.

|===

== Trike - Untrusted Actor With Incoming Communication Links Without Authorised Read
[.small]#trike-unauthorized-read#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | In Trike's rights-based model, every actor must have explicitly authorised read access to the assets it accesses. A technical asset tagged with trike-actor-untrusted that receives incoming communication links but lacks the trike-read-authorized tag has not had its read rights formally approved. Untrusted actors with uninspected read paths can harvest data from assets they should not be permitted to access, representing a failure of the intended action matrix.

| Detection:   | In-scope technical assets tagged with trike-actor-untrusted that have at least one incoming communication link and are not tagged with trike-read-authorized.

| Rating:      | High baseline. Critical when incoming links carry confidential or personally identifiable data that the untrusted actor should not access.
