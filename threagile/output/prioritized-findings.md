...
| 6 | 66 | elevated | Missing Hardening risk at PostgreSQL Database | PostgreSQL Database |
| 7 | 66 | elevated | Path-Traversal risk at API Server against filesystem MinIO Object Storage via MinIO File Storage | API Server |
| 8 | 66 | elevated | Unencrypted Communication named MinIO File Storage between API Server and MinIO Object Storage transferring authentication data (like credentials, token, session-id, etc.) | API Server |
| 9 | 66 | elevated | Unencrypted Communication named PostgreSQL Connection between API Server and PostgreSQL Database transferring authentication data (like credentials, token, session-id, etc.) | API Server |
| 10 | 66 | elevated | Unencrypted Communication named Redis Session Store between API Server and Redis Cache transferring authentication data (like credentials, token, session-id, etc.) | API Server |
