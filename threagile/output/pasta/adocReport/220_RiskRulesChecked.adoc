...
| Threagile Execution Timestamp: | 20260604113022
...
== Stage IV - Unencrypted Traffic on Internet-Facing Communication Link
[.small]#unencrypted-external-traffic#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | PASTA Stage IV analyses threats against identified entry points. Any communication link crossing the internet boundary without TLS exposes data to interception and tampering. An attacker on any hop between client and server can read credentials, session tokens, and business data, or inject malicious content.

| Detection:   | Communication links originating from internet-exposed technical assets that use HTTP, a plain database protocol, or another non-TLS protocol.

| Rating:      | High; carrying credentials or sensitive data over an unencrypted internet-facing link is a critical misconfiguration.
...
== Stage V - Outdated TLS Version on External Communication Link
[.small]#outdated-tls-version#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | PASTA Stage V identifies weaknesses. TLS 1.0 and 1.1 are deprecated and known-broken (BEAST, POODLE, DROWN attacks). Using them on internet-facing communication links leaves those links vulnerable to downgrade attacks and known cipher weaknesses. PCI-DSS 3.2, NIST SP 800-52 Rev 2, and FIPS 140-2 all require TLS 1.2+.

| Detection:   | Communication links from internet-exposed assets where tls_version is set to 1.0 or 1.1, indicating explicitly declared outdated TLS.

| Rating:      | High for any link that carries sensitive data; elevated for all links since even public data integrity is at risk from tampering.
...
== Stage VII - Internet-Reachable Datastore With No Mitigation
[.small]#internet-reachable-datastore#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | PASTA Stage VII evaluates residual risk. A datastore (database, cache, object store) that can be reached from the internet, either directly or via a chain of HTTP/plain protocol hops, with no network segmentation represents one of the highest-risk configurations in any architecture. Attackers who compromise any internet-facing component gain a direct path to sensitive stored data.

| Detection:   | In-scope datastore technical assets that have any incoming communication link originating from an internet-exposed technical asset.

| Rating:      | Critical when the datastore holds strictly-confidential data; high otherwise.
...
== Stage II - Custom-Developed Asset With No Build Pipeline Security
[.small]#custom-development-no-sast#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | PASTA Stage II defines the technical scope and identifies custom-developed components that require code-level security analysis. A custom-developed technical asset without a build pipeline performing Static Application Security Testing (SAST) means that code vulnerabilities are not caught before deployment, increasing the probability that known vulnerability classes survive to production.

| Detection:   | In-scope custom-developed technical assets where no sourcecode repository or build-pipeline asset exists in the model to provide build security controls.

| Rating:      | Medium baseline; elevated for internet-exposed custom-developed assets processing sensitive data.
...
== Stage VI - Weak Authentication Strength on Internet-Exposed Entry Point
[.small]#weak-auth-on-entry-point#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Spoofing
| Description: | PASTA Stage VI models how attackers simulate attacks against identified entry points. Internet-exposed entry points with weak authentication (none or password-only) are prime targets for credential stuffing, phishing, and account takeover attacks. High-value or sensitive entry points should require MFA or hardware tokens.

| Detection:   | Internet-exposed technical assets classified as entry points where requires_authentication_strength is not mfa or hardware and the asset handles confidential or strictly-confidential data.

| Rating:      | High for entry points storing or processing strictly-confidential data with no MFA. Elevated for confidential data with password-only authentication.
...
== Stage III - Undocumented Application Entry Point
[.small]#undocumented-entry-point#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | PASTA Stage III requires decomposing the application into entry points so that threats can be mapped against known attack surfaces. A technical asset reachable from the internet or marked as internet-exposed that has no entry_point_type declared represents an undocumented entry point that cannot be systematically analysed for threats.

| Detection:   | In-scope technical assets marked internet=true or reachable from an internet trust boundary that do not have an entry_point_type field declared.

| Rating:      | Medium baseline; elevated when the asset processes sensitive data or requires no authentication.

|===

== Stage V - Custom-Developed Service Without Per-Operation Rate Limiting
[.small]#per-operation-rate-limiting-missing#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Denial of Service
| Description: | PASTA Stage V identifies vulnerabilities in the attack surface. Custom-developed services that process authentication credentials or session tokens and have outgoing communication links without rate limiting are vulnerable to credential stuffing, brute-force, bulk data exfiltration, and resource exhaustion. Global API rate limits are too coarse to prevent targeted abuse of individual high-value operations such as login, password reset, or bulk data exports.

| Detection:   | In-scope custom-developed process-type technical assets that process authentication data (user-credentials or session-tokens) and have at least one outgoing communication link where rate_limited is false and usage is not devops.

| Rating:      | Medium baseline. Elevated for assets with internet-exposed entry points or those processing strictly-confidential authentication credentials.
...
== Stage I - No Threat Scenarios Defined for Internet-Exposed Entry Points
[.small]#missing-threat-scenario#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | PASTA Stage I defines business and security objectives and maps them to threat actors. An architecture with internet-exposed entry points but no documented threat scenarios means that adversarial thinking has not been applied: the team has not specified who might attack, how, and with what capabilities.

| Detection:   | Internet-exposed technical assets with a declared entry_point_type but no matching threat scenario that lists them as an entry asset.

| Rating:      | Low severity from a technical standpoint but indicates a process gap that increases risk of systematic coverage omissions.
...
== Stage V - No Rate Limiting on Internet-Exposed API
[.small]#no-rate-limiting-public-api#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Denial of Service
| Description: | PASTA Stage V identifies weaknesses that enable exploitation. An internet-accessible API endpoint without rate limiting is vulnerable to brute-force credential attacks, credential stuffing, API abuse, and denial-of-service amplification. Rate limiting is a fundamental control for any internet-exposed API.

| Detection:   | In-scope technical assets with entry_point_type of api or web_ui that receive communication links from internet-exposed assets where rate_limited is false.

| Rating:      | Elevated for authentication endpoints; medium for general data APIs. High when the API has no authentication at all.
...
== Stage III - Asset Exposed With Internet-Level Attack Surface
[.small]#wide-attack-surface-exposure#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | PASTA Stage III decomposes the application to understand what is exposed and to whom. A technical asset with attack_surface_exposure set to internet and type process or datastore that also lacks explicit authentication on its ingress links represents an unnecessarily wide attack surface that should be minimised per least-privilege network access principles.

| Detection:   | In-scope process or datastore technical assets where attack_surface_exposure is internet and at least one incoming link has authentication of none.

| Rating:      | High for datastores; elevated for processes. Critical when combined with sensitive data.
