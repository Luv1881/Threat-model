...
== Detectability - PII Flows Without Audit Logging
[.small]#detecting-no-audit-logging#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Repudiation
| Description: | Communication links that carry personal data without audit logging enabled mean that access to and use of personal data cannot be detected or reviewed after the fact. GDPR Article 5(2) requires demonstrable accountability.

| Detection:   | In-scope technical assets that process PII AND have at least one outgoing link without audit logging enabled.

| Rating:      | Medium severity baseline; elevated if the data includes sensitive categories.
|===
...
[.small]#unawareness-no-consent-technology#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | When a system processes personal data from end users but has no consent-management technology in the architecture, data subjects are likely unaware of the nature and extent of processing. GDPR Articles 13 and 14 require transparent disclosure.

| Detection:   | In-scope user-facing technical assets that process PII data assets where no other asset carries a consent_collector technology attribute.

| Rating:      | Medium baseline; elevated for consumer-facing applications or special-category data.
|===
...
== Non-Compliance - Personal Data Processed Without Lawful Basis
[.small]#non-compliance-no-lawful-basis#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | GDPR Article 6 requires that every processing activity for personal data has a declared lawful basis. A technical asset that processes PII data assets with no declared lawful_basis on any of those data assets is non-compliant.
| Detection:   | In-scope technical assets that process at least one data asset with declared pii_categories but empty lawful_basis.
| Rating:      | Processing without a lawful basis is a strict GDPR violation; severity scales with the sensitivity of the personal data.
|===
...
== Non-Compliance - Cross-Border Personal Data Transfer Without Safeguard
[.small]#non-compliance-cross-border-transfer#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | GDPR Chapter V restricts transfers of personal data to third countries. A communication link marked cross_border as true that also involves PII data assets indicates a potential cross-border transfer that must be covered by an adequacy decision, SCCs, BCRs, or another Article 46 safeguard.

| Detection:   | In-scope technical assets that process PII AND have at least one outgoing link with cross_border set to true.

| Rating:      | Elevated baseline; critical for transfers to countries without adequacy decisions.
|===

== Non-Repudiation - PII or Auth Tokens Accessible in Browser-Side Storage
[.small]#pii-client-side-storage#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Browser-based clients (SPAs, web apps) that process personal data or authentication tokens typically persist them in localStorage or sessionStorage. This browser-accessible storage is readable by any JavaScript running in the same origin — including injected scripts. Without HttpOnly cookies or a strict Content-Security-Policy, the stored tokens and PII are one XSS payload away from full exfiltration. From a LINDDUN perspective this enables Non-Repudiation attacks: an adversary who steals a token can act as the victim, making it impossible to distinguish attacker actions from legitimate user actions in audit logs.

| Detection:   | In-scope technical assets with browser (client) technology that process PII data assets and are not tagged with has-httponly-session.

| Rating:      | Medium baseline. Elevated when tokens have long TTL (>1 hour) or when no CSP is configured on the serving origin.
...
== Non-Repudiation - PII Datastore Lacks Audit-Logged Ingress
[.small]#non-repudiation-missing-access-log#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Repudiation
| Description: | A datastore that holds personal data whose incoming communication links lack audit logging allows access to personal data without any tamper-evident record. This violates the GDPR accountability principle (Article 5(2)).

| Detection:   | In-scope datastore technical assets storing PII data assets that also have at least one incoming communication link without audit logging enabled.

| Rating:      | Medium baseline; elevated for highly sensitive or regulated personal data.
|===
...
== Identifiability - Personal Data Stored Without Encryption at Rest
[.small]#identifying-pii-stored-unencrypted#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Datastores holding personal data without encryption at rest allow anyone with access to the underlying storage to read and identify data subjects. GDPR Article 32 requires appropriate security measures including encryption at rest.

| Detection:   | In-scope datastore technical assets storing at least one PII data asset with the encryption field set to none.

| Rating:      | Severity matches the confidentiality classification of the PII stored. Strictly confidential PII warrants Critical.
...
== Data Disclosure - Personal Data Transmitted Over Unencrypted Channel
[.small]#data-disclosure-pii-unencrypted-link#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Personal data in transit over unencrypted communication links (plain HTTP, unencrypted database protocols, etc.) is exposed to network observers. GDPR Article 32 requires appropriate technical measures including encryption in transit.
| Detection:   | In-scope technical assets that process PII data assets AND have at least one outgoing communication link that is unencrypted and not VPN-protected.
| Rating:      | High — unencrypted PII in transit is a direct GDPR Article 32 violation.
|===
...
[.small]#linking-pii-multi-boundary#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | When the same personal-data asset is processed by technical assets in two or more different trust boundaries, an attacker who compromises any one boundary gains linkability across all others, enabling cross-context profiling.

| Detection:   | Technical assets that process PII data assets that also appear in assets in at least one other trust boundary.

| Rating:      | Medium baseline; elevated for sensitive categories or high-quantity data.
|===
