...
| Threagile Execution Timestamp: | 20260604113045
...
== Cloud Container - Container Platform Without Image Vulnerability Scanning
[.small]#cloud-container-no-image-scanning#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Container platforms running images that are not scanned for known CVEs regularly ship vulnerable base images and application dependencies into production. Container image scanning (Trivy, Snyk, AWS ECR scanning, GCP Artifact Registry scanning) identifies publicly disclosed vulnerabilities in OS packages and language libraries before they are deployed, providing a critical defense-in-depth layer against known exploits.

| Detection:   | In-scope container-platform technical assets not tagged with has-image-scanning.

| Rating:      | Medium baseline. Elevated to High when the container platform runs internet-facing workloads or processes confidential data.

|===

== Cloud Data - Managed Database Directly Internet-Exposed
[.small]#cloud-managed-db-public-exposure#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Managed cloud databases should never be accessible directly from the internet. When a managed database (RDS, Cloud SQL, CosmosDB) is assigned a public IP or is not confined to a private VPC, SQL and NoSQL attack surfaces are exposed directly to the internet, enabling brute-force of credentials, exploitation of unpatched engine vulnerabilities, and direct exfiltration of data.

| Detection:   | In-scope technical assets with managed-database technology and internet: true.

| Rating:      | Critical for any managed database regardless of data sensitivity, because direct internet exposure eliminates the VPC isolation layer.
...
== Cloud Data - Managed Database Without Automated Backup
[.small]#cloud-managed-db-no-backup#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Denial of Service
| Description: | Managed databases without automated backup and point-in-time recovery (PITR) are vulnerable to permanent data loss from accidental deletion, ransomware encryption, and destructive SQL injection. Automated daily snapshots and transaction-log-based PITR are available on all major cloud-managed database services and should be enabled for any database processing business data.

| Detection:   | In-scope technical assets with managed-database technology that store or process confidential data and are not tagged with has-automated-backup.

| Rating:      | High when the database is the authoritative source for business-critical data. Medium for read replicas or caches with upstream recovery paths.

|===

== Cloud Storage - Object Storage Without Versioning for Strictly-Confidential Data
[.small]#cloud-object-storage-no-versioning#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Cloud object storage that holds strictly-confidential data without versioning enabled is vulnerable to irreversible data loss and ransomware. An attacker who gains write access can overwrite or delete objects with no recovery path. Versioning combined with object lock (WORM) prevents deletion of authoritative copies and enables recovery from accidental or malicious overwrites.

| Detection:   | In-scope technical assets with object-storage technology storing at least one strictly-confidential data asset and not tagged with has-versioning.

| Rating:      | Medium baseline. Elevated to High when the data has no other backup mechanism and availability is rated critical.

|===

== Cloud Network - Internet-Facing API Gateway Without WAF Protection
[.small]#cloud-api-gateway-no-waf#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | Cloud API gateways that serve internet traffic without a Web Application Firewall (WAF) have no automated protection against common web exploits including SQL injection, XSS, path traversal, and HTTP floods. A WAF inspects request payloads and headers before they reach backend services, providing a defense-in-depth layer that can block known attack patterns in real-time.

| Detection:   | In-scope technical assets with cloud-api-gateway technology that are internet-accessible and not tagged with has-waf.

| Rating:      | High when the API processes confidential data or authentication tokens. Medium for public APIs serving only non-sensitive data.
...
== Cloud Audit - High-Value Cloud Service Without Audit Logging
[.small]#cloud-no-audit-logging-high-value#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Repudiation
| Description: | High-value cloud services (managed databases, object storage, serverless functions, API gateways) that do not have audit logging enabled make it impossible to detect unauthorized access, investigate breaches, or meet compliance requirements. Cloud audit logs (CloudTrail, GCP Cloud Audit Logs, Azure Monitor) capture who accessed what resource, when, and from where, providing the forensic foundation for incident response.

| Detection:   | In-scope technical assets with managed-database, object-storage, serverless-function, or cloud-api-gateway technology that process confidential data and are not tagged with has-audit-logging.

| Rating:      | High when the service handles strictly-confidential data. Medium for confidential data without direct regulatory exposure.

|===

== Cloud Storage - Object Storage Without Encryption at Rest
[.small]#cloud-object-storage-no-encryption#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Cloud object storage buckets that store sensitive or personal data without server-side encryption at rest allow any actor with storage-level access (via cloud-provider support channels, storage engineer, or leaked keys) to read the raw data. GDPR Article 32 and most compliance frameworks require encryption at rest as a baseline technical safeguard for personal and sensitive data.

| Detection:   | In-scope technical assets with object-storage technology, encryption set to none, that store or process at least one confidential or higher data asset.

| Rating:      | High when the bucket stores confidential or strictly-confidential data. Medium for internal-use-only data without external exposure.
...
== Cloud Container - Container Registry Publicly Accessible
[.small]#cloud-container-registry-public#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Container image registries (ECR, GCR, ACR) that are publicly accessible allow any internet actor to pull images from the registry. Container images frequently contain embedded application code, configuration files with connection strings, and in some cases hardcoded secrets. Public pull access also enables an attacker to analyze image layers for vulnerabilities and plan targeted exploits against the production workload.

| Detection:   | In-scope technical assets with container-registry technology that are internet-accessible and not tagged with has-private-registry.

| Rating:      | High when images contain proprietary code or application configuration. Medium for base-image-only registries with no application logic.
...
== Cloud Data - Managed Database Without Encryption at Rest
[.small]#cloud-managed-db-no-encryption#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Managed cloud databases (RDS, Aurora, Cloud SQL, CosmosDB) without transparent data encryption (TDE) store table data, indexes, and backups in plaintext on the underlying storage tier. A provider-side breach, a misconfigured snapshot, or a storage engineer with direct volume access can expose all records. Most compliance regimes (PCI-DSS, HIPAA, GDPR) require encryption at rest for sensitive and personal data.

| Detection:   | In-scope technical assets with managed-database technology and encryption set to none.

| Rating:      | High when the database holds confidential or strictly-confidential data. Medium for internal data where external exposure risk is low.

|===

== Cloud Serverless - Function Execution Role Without Least-Privilege Restrictions
[.small]#cloud-serverless-overpermissive-role#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Serverless functions attached to overly permissive execution roles (e.g., an IAM role with AdministratorAccess or * actions on all resources) violate the principle of least privilege. If the function is compromised via a code injection vulnerability or a supply-chain attack on a dependency, the attacker inherits the full permissions of the execution role, potentially gaining control of the entire cloud account.

| Detection:   | In-scope serverless function technical assets processing confidential data that are not tagged with has-restricted-execution-role.

| Rating:      | High when the function processes confidential data or calls downstream services with broad permissions. Medium for functions processing only public data.

|===

== Cloud Container - Privileged Containers Running on Platform
[.small]#cloud-container-privileged-workloads#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Containers launched with the privileged flag have the same capabilities as the host kernel's root process. A privileged container can mount the host filesystem, load kernel modules, modify network namespaces, and escape the container boundary entirely. Running privileged containers breaks the isolation guarantee that containers are supposed to provide.

| Detection:   | In-scope container-platform technical assets tagged with has-privileged-containers.

| Rating:      | High baseline. Critical when the container platform is multi-tenant or runs workloads processing strictly-confidential data.
...
== Cloud Network - API Gateway Without Rate Limiting
[.small]#cloud-api-gateway-no-rate-limiting#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Denial of Service
| Description: | API gateways without rate limiting expose backend services to unbounded request floods that can exhaust serverless invocation quotas, trigger compute autoscaling cost amplification, and overwhelm downstream databases. Rate limiting at the API gateway layer also prevents credential stuffing by throttling high-frequency authentication attempts from individual IP addresses or API keys.

| Detection:   | In-scope cloud-api-gateway technical assets not tagged with has-rate-limiting.

| Rating:      | Medium baseline. Elevated when the gateway fronts serverless functions billed per-invocation, or when it handles authentication endpoints.

|===

== Cloud IAM - Long-Lived API Keys or Access Credentials Detected
[.small]#iam-long-lived-credentials#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Long-lived API keys and access credentials (static IAM access key IDs and secrets) are a leading cause of cloud credential compromise. Unlike short-lived credentials issued by IAM roles and instance profiles, static keys do not expire and are frequently leaked through code repositories, container images, logs, and misconfigured storage. An attacker with a static key has indefinite access until the key is manually revoked.

| Detection:   | In-scope technical assets tagged with has-long-lived-api-key that do not also carry the has-key-rotation tag.

| Rating:      | High when the asset handles sensitive data or has broad cloud permissions. Medium when usage is scoped to read-only or non-sensitive operations.
...
== Cloud Serverless - Function Without Dead Letter Queue for PII Processing
[.small]#cloud-serverless-no-dlq#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Denial of Service
| Description: | Serverless functions that process personal data without a dead letter queue (DLQ) silently drop failed events after the maximum retry count is exhausted. Silent failure means PII events are permanently lost with no audit trail, violating GDPR data integrity requirements and making forensic investigation after an incident impossible. A DLQ captures failed payloads for investigation and replay, maintaining data integrity and enabling incident response.

| Detection:   | In-scope serverless function technical assets processing PII data assets without the has-dlq tag.

| Rating:      | Medium baseline. Elevated when the function is the only processor for PII with no alternative audit path.

|===

== Cloud IAM - Internet-Exposed Identity Provider Without MFA
[.small]#iam-internet-identity-no-mfa#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Spoofing
| Description: | Identity providers and identity stores that are accessible from the internet without multi-factor authentication (MFA) are exposed to credential stuffing, phishing, and brute-force attacks. In cloud architectures, compromising the identity plane gives attackers access to every service the principal can reach, including storage, compute, databases, and cross-account resources.

| Detection:   | In-scope technical assets whose technology is identity-provider or identity-store-database that are internet-accessible and not tagged with has-mfa.

| Rating:      | High baseline. Critical when the identity provider manages cross-account or administrative access. Elevated for any identity store processing confidential data.

|===

== Cloud Storage - Object Storage Bucket with Public Internet Access
[.small]#cloud-object-storage-public-access#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | Cloud object storage buckets (S3, GCS, Azure Blob) that are accessible from the internet without access controls are a primary cause of large-scale cloud data breaches. Default bucket policies vary by provider but misconfigurations that enable public ListBucket or GetObject permissions expose all stored content to unauthenticated retrieval by any internet actor.

| Detection:   | In-scope technical assets with object-storage technology that are internet-exposed (internet: true) and not tagged with has-private-acl.

| Rating:      | Critical when the bucket stores confidential or strictly-confidential data. High for all other publicly accessible buckets due to data enumeration risk.
...
== Cloud Serverless - Publicly Accessible Function Without Authentication
[.small]#cloud-serverless-public-no-auth#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | Serverless functions (Lambda, Cloud Functions, Azure Functions) with public URLs or API Gateway triggers that require no authentication expose the function's full capability to unauthenticated internet actors. An unauthenticated function that processes business logic, stores data, or calls downstream services represents a direct path for unauthorized access and data manipulation.

| Detection:   | In-scope technical assets with serverless-function technology that are internet-accessible, have an entry_point_type set, and do not have requires_authentication_strength set to password, mfa, or hardware.

| Rating:      | High when the function processes or returns confidential data. Medium when the function has limited capability but could be abused for DoS.
