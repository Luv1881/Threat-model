...
| Threagile Execution Timestamp: | 20260604113024
...
== Operational Threat - Critical Business Process Supported by Non-Redundant Assets
[.small]#operational-critical-process-no-redundancy#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Denial of Service
| Description: | VAST Operational Threat Model: a business process classified as mission-critical or critical that is supported exclusively by technical assets with redundant=false. A single point of failure in the supporting infrastructure can take down the entire critical process, failing service level agreements and potentially causing significant financial or safety impact.

| Detection:   | Business processes classified as mission-critical or critical where all supporting technical assets have redundant set to false.

| Rating:      | Critical for mission-critical processes; high for critical processes without redundancy.

|===

== Operational Threat - Critical Business Process on Shared Runtime
[.small]#operational-shared-runtime-critical-process#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Elevation of Privilege
| Description: | VAST Operational Threat Model: a business process marked as mission-critical or critical that depends on a shared runtime (container host, VM cluster) with other, lower-criticality workloads. A compromise of any co-located workload (container escape, host privilege escalation) can directly impact the critical process through shared kernel, storage, or network namespaces.

| Detection:   | Shared runtimes that host assets supporting mission-critical or critical business processes together with assets that do not support any business process.

| Rating:      | High when a critical business process asset shares a runtime with an internet-facing component; elevated otherwise.
...
== Application Threat - Business Process Entry Point Lacks Authentication
[.small]#application-business-process-no-auth#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Spoofing
| Description: | VAST Application Threat Model: a technical asset that supports a business process and is directly reachable from the internet but receives unauthenticated connections. Any actor on the internet can interact with this business process without proving their identity, enabling fraud, abuse, and data manipulation.

| Detection:   | Internet-exposed technical assets that support at least one business process and receive at least one unauthenticated incoming communication link.

| Rating:      | High; unauthenticated business process access is a fundamental design flaw.
...
== Application Threat - No Input Validation on Customer-Facing Asset
[.small]#application-no-input-validation#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Tampering
| Description: | VAST Application Threat Model: a custom-developed asset that is directly accessible by human users and accepts structured data formats (JSON, XML, file uploads) without any explicit declaration of input validation controls. Absence of input validation is the root cause of injection families (SQLi, XSS, XXE, command injection, path traversal) and is consistently in the OWASP Top 10.

| Detection:   | In-scope custom-developed assets used as a client by humans that accept JSON, XML, or file input formats.

| Rating:      | High for assets accepting file or XML (XXE risk); medium for JSON-only endpoints that rely on parameterised queries.

|===

== Application Threat - Business Process Has No Declared Owner
[.small]#application-process-without-business-owner#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Repudiation
| Description: | VAST Application Threat Model: a business process with no declared owner. Without an owner, no one is accountable for accepting residual risk, driving mitigations, reviewing access controls, or approving changes that affect the process. This is a governance gap that enables threats to go unaddressed through organisational inertia.

| Detection:   | Business processes in the model that have no owner declared.

| Rating:      | Low technical severity but indicates a governance gap that elevates residual risk across the board.
...
== Operational Threat - Critical Business Process on Multi-Tenant Asset
[.small]#operational-multi-tenant-critical-process#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | VAST Operational Threat Model: a technical asset that supports a critical business process but is deployed in multi-tenant mode. In a multi-tenant deployment, a tenant isolation failure, noisy-neighbour attack, or metadata leak can expose the critical process to data belonging to another tenant or to interference that degrades availability.

| Detection:   | Multi-tenant technical assets that support critical or mission-critical business processes.

| Rating:      | High for mission-critical processes; medium for critical processes where tenant isolation controls are well-documented.

|===

== Operational Threat - Critical Business Process Without Operational Monitoring
[.small]#operational-unmonitored-critical-process#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Repudiation
| Description: | VAST Operational Threat Model: a business process classified as critical or mission-critical whose supporting technical assets have no communication links with audit logging enabled. Without monitoring, security incidents within this process go undetected, incident response is hindered, and compliance with accountability requirements (GDPR Art. 5(2), SOC 2, ISO 27001 A.12.4) is undermined.

| Detection:   | Technical assets supporting critical or mission-critical business processes that have no incoming communication link with audit_logged set to true.

| Rating:      | High; critical processes without monitoring violate multiple compliance frameworks.

|===

== Application Threat - Strictly-Confidential Asset Bridges Business Process Contexts
[.small]#application-secrets-across-processes#


[cols="h,1",frame=none,grid=none]
|===
| STRIDE:      | Information Disclosure
| Description: | VAST Application Threat Model: a technical asset that supports business processes and stores or processes strictly-confidential data. Assets that bridge process contexts while holding sensitive credentials or secrets expose those secrets to any threat actor that targets any one of the supported processes, violating the least-privilege boundary between business processes.

| Detection:   | In-scope technical assets that support at least one business process AND process or store strictly-confidential data.

| Rating:      | High when the asset is internet-accessible; medium for internal-only assets.
